...
FUNCTION LS1RB (YLF, NAME,LDACC,LDRDONLY)
...
FUNCTION LS2RB (YLF,NAME,LDACC,LDRDONLY)
...
FUNCTION LS3RB (YLF,NAME,LDACC,LDRDONLY)
  ...
  DO JFLD = 1, SIZE (YLFL)
   ...
   ! Phase 2: call the correct routine to create the SPEC_VIEW for each field of the list
    SELECT TYPE (YLF => YLFL (JFLD)%PTR)
      CLASS IS (FIELD_1RB)
        ILEN = 1
        IF (JPASS == 2) LS (IOFF+1:IOFF+ILEN) = LS1RB (YLF,YLFL(JFLD)%NAME, LDACC,LDRDONLY)
      CLASS IS (FIELD_2RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
        ILEN = (IUBOUNDS (1) - ILBOUNDS (1) + 1)
        IF (JPASS == 2) LS (IOFF+1:IOFF+ILEN) = LS2RB (YLF,YLFL(JFLD)%NAME, LDACC,LDRDONLY)
      CLASS IS (FIELD_3RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
         ILEN =  (IUBOUNDS (1) - ILBOUNDS (1) + 1)* (IUBOUNDS (3) - ILBOUNDS (3) + 1)
        IF (JPASS == 2) LS (IOFF+1:IOFF+ILEN) = LS3RB (YLF ,YLFL(JFLD)%NAME,LDACC,LDRDONLY)
      CLASS IS (FIELD_4RB)
