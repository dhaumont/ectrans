   ...
   ! Phase 2: call the correct routine to create the GRID_VIEW for each field of the list
    SELECT TYPE (YLF => YLFL (JFLD)%PTR)
      ...
      CLASS IS (FIELD_2RB)
        ...
        ILEN = 1
        IF (JPASS == 2) LG (IOFF+1:IOFF+ILEN) = LG2RB (YLF,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET,LDACC,LDRDONLY)
      CLASS IS (FIELD_3RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
        ILEN = (IUBOUNDS (2) - ILBOUNDS (2) + 1)
        IF (JPASS == 2) LG (IOFF+1:IOFF+ILEN) = LG3RB (YLF,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET, LDACC,LDRDONLY)
      CLASS IS (FIELD_4RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
