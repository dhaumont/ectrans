...
FUNCTION LG2RB (YLF,NAME,IVSET,LDACC,LDRDONLY)
...
FUNCTION LG3RB (YLF,NAME, IVSET,LDACC,LDRDONLY)
...
FUNCTION LG4RB (YLF,NAME,IVSET,LDACC,LDRDONLY)
   ...
   ! Phase 2: call the correct routine to create the GRID_VIEW for each field of the list
    SELECT TYPE (YLF => YLFL (JFLD)%PTR)
      ...
      CLASS IS (FIELD_2RB)
        ...
        ILEN = 1
        IF (JPASS == 2) LGV (IOFF+1:IOFF+ILEN) = LG2RB (YLF,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET,LDACC,LDRDONLY)
      CLASS IS (FIELD_3RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
        ILEN = (IUBOUNDS (2) - ILBOUNDS (2) + 1)
        IF (JPASS == 2) LGV (IOFF+1:IOFF+ILEN) = LG3RB (YLF,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET, LDACC,LDRDONLY)
      CLASS IS (FIELD_4RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
        ILEN = (IUBOUNDS (2) - ILBOUNDS (2) + 1) * (IUBOUNDS (3) - ILBOUNDS (3) + 1)
        IF (JPASS == 2) LGV (IOFF+1:IOFF+ILEN) = LG4RB (YLF,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET,LDACC,LDRDONLY)
      CLASS DEFAULT
  ...
  DO JFLD = 1, SIZE (YLFL)
   ...
   ! Phase 2: call the correct routine to create the SPEC_VIEW for each field of the list
    SELECT TYPE (YLF => YLFL (JFLD)%PTR)
      CLASS IS (FIELD_1RB)
        ILEN = 1
        IF (JPASS == 2) LSV(IOFF+1:IOFF+ILEN) = LS1RB (YLF,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET, LDACC,LDRDONLY)
      CLASS IS (FIELD_2RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
        ILEN = (IUBOUNDS (1) - ILBOUNDS (1) + 1)
        IF (JPASS == 2) LSV(IOFF+1:IOFF+ILEN) = LS2RB (YLF,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET, LDACC,LDRDONLY)
      CLASS IS (FIELD_3RB)
        CALL YLF%GET_DIMS (LBOUNDS=ILBOUNDS, UBOUNDS=IUBOUNDS)
         ILEN =  (IUBOUNDS (1) - ILBOUNDS (1) + 1)* (IUBOUNDS (3) - ILBOUNDS (3) + 1)
       IF (JPASS == 2) LSV(IOFF+1:IOFF+ILEN) = LS3RB (YLF ,YLFL(JFLD)%NAME,YLFL(JFLD)%IVSET, LDACC,LDRDONLY)
      CLASS IS (FIELD_4RB)
...
SUBROUTINE COPY_DEVICE_1RB (PTR_LHS, PTR_RHS)
...
SUBROUTINE COPY_DEVICE_2RB (PTR_LHS, PTR_RHS)
