...
INTEGER(C_INT), POINTER :: IVSET_PAIRED(:)
...
IF (SIZE(YDGPU) > 0) THEN
 ...
 ENDIF
  ...
  ! Grid-point FIELD_VIEW does not carry IVSET information.
  ! Propagate IVSET from the paired spectral field (YDSPVOR(JFLD)) into YLGVU / YLGVV.
  DO JFLD=1,IUVG
    CALL GET_IVSET(YDSPVOR(JFLD), IVSET_PAIRED)
    IF (SIZE(IVSET_PAIRED) /= KFLEVG) &
      & CALL ABOR1("[INV_TRANS_FIELD_API] YDSPVOR ivset size inconsistent with KFLEVG")
    DO JLEV=1,KFLEVG
      ID = JLEV + (JFLD - 1) * KFLEVG
      YLGVU(ID)%IVSET = IVSET_PAIRED(JLEV)
      YLGVV(ID)%IVSET = IVSET_PAIRED(JLEV)
    ENDDO
  ENDDO

  ! Build IVSETUV (per-level b-set map). Must be consistent across fields.
  DO JFLD=1,IUVG
    DO JLEV=1,KFLEVG
      ID = JLEV + (JFLD - 1) * KFLEVG
      IF (JFLD .EQ. 1) IVSETUV(JLEV) = YLGVU(ID)%IVSET
      IF (IVSETUV(JLEV) .NE. YLGVU(ID)%IVSET) &
        & CALL ABOR1("[INV_TRANS_FIELD_API] ivsetuv inconsistent with ylgvu%ivset")
    ENDDO
  ENDDO
ELSE
...
! allocate 'b-set' for scalar fields
 ...
 ! compute ´b-set´ for scalar-fields
  ...
  ! Grid-point FIELD_VIEW does not carry IVSET information.
  ! Propagate IVSET from the paired spectral field YDSPSCALAR into YLGVSCALAR.
  ID = 0
  DO JFLD = 1, SIZE(YDSPSCALAR)
    CALL GET_IVSET(YDSPSCALAR(JFLD), IVSET_PAIRED)
    DO JLEV = 1, SIZE(IVSET_PAIRED)
      ID = ID + 1
      IF (ID > IFLDXG) &
        & CALL ABOR1("[INV_TRANS_FIELD_API] more ivset entries than grid scalar views")
      YLGVSCALAR(ID)%IVSET = IVSET_PAIRED(JLEV)
    ENDDO
  ENDDO
  IF (ID /= IFLDXG) &
    & CALL ABOR1("[INV_TRANS_FIELD_API] ivset count mismatch between YDSPSCALAR and YDGPSCALAR")
...
IF (IUVG>0) THEN
  ...
  IF (LLUVDER) THEN
    C = LG(YDGPU_EW, YLGVU_EW)
    C = LG(YDGPV_EW, YLGVV_EW)
...
IF (IFLDXG > 0) THEN
  ...
  IF (LLSCDERS) THEN
    C = LG(YDGPSCALAR_NS, YLGVSCALAR_NS)
    C = LG(YDGPSCALAR_EW, YLGVSCALAR_EW)
