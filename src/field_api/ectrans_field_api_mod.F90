...
ILEN = -1
  ...
  WRITE(NOUT,*) "LS BEGIN: number of fiels inv-trans ", SIZE(YLFL)
! iterate over YLFL LIST
DO JFLD = 1, SIZE (YLFL)
  ...
  SELECT TYPE (YLF => YLFL (JFLD)%PTR)
    ...
    CLASS IS (FIELD_3RB)
      ...
      WRITE(NOUT,*) " ADD FIELD_3RB ", ILEN
    CLASS IS (FIELD_4RB)
        WRITE(NOUT,*) " ADD FIELD_4RB ", ILEN
        CALL ABOR1("LS not implemeted for FIELD_4RB")
    CLASS DEFAULT
        ! Skip the spectral field as it is not present on this processor
        ILEN = 0
        WRITE(NOUT,*) " ADD DEFAULT ", ILEN
  END SELECT

  IOFF = IOFF + ILEN
ENDDO
WRITE(NOUT,*) "LS END: IOFF is ", IOFF
