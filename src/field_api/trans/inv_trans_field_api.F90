...
IF (LDACC) THEN
  !$ACC KERNELS PRESENT(ZPGPUV, ZPGP2) FIRSTPRIVATE(IEND, KGPBLKS)
...
IF (IUVG>0) THEN
    ...
    ! copy u and v
    IF (LDACC) THEN
      !$ACC KERNELS PRESENT(ZPGPUV,YYLGVU(:)%VIEW%P,LGVV(:)%VIEW%P) FIRSTPRIVATE(IUVG, KFLEVG,IOFFSET) PRIVATE(ID,JFLD,JLEV)
      DO JFLD=1,IUVG
        DO JLEV=1,KFLEVG
          ID = JLEV + (JFLD -1) * KFLEVG
