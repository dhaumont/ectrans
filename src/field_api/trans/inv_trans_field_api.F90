...
IF (PRESENT(YDFU)) THEN
  ...
  IF (LDACC) THEN
    !$ACC KERNELS PRESENT(ZPSPVOR,ZPSPDIV,YLSPVVOR(:)%VIEW%P,YLSPVDIV(:)%VIEW%P) COPY(IFLDSPVOR)
...
! allocate 'b-set' for scalar fields
  ...
  IF (LDACC) THEN
    !$ACC KERNELS PRESENT(ZPSPSC2,YLSPVSCALAR(:)%VIEW%P)  COPY(IFLDSPSC)
    ID = 1
    DO JFLD = 1,IFLDSPSC
      IF (ASSOCIATED(YLSPVSCALAR(JFLD)%VIEW%P)) THEN
...
IF (IUVG>0) THEN
    ...
    IF (LLVORGP) THEN
      IF (LDACC) THEN
        !$ACC KERNELS PRESENT(ZPGPUV,YLGVVOR(:)%VIEW%P) COPY(IUVG, KFLEVG,IOFFSET)
        DO JFLD=1,IUVG
          DO JLEV=1,KFLEVG
            ID = JLEV + (JFLD -1) * KFLEVG
    ...
    IF (LLDIVGP) THEN
      IF (LDACC) THEN
        !$ACC KERNELS PRESENT(ZPGPUV,YLGVDIV(:)%VIEW%P) COPY(IUVG, KFLEVG,IOFFSET)
        DO JFLD=1,IUVG
          DO JLEV=1,KFLEVG
          ID = JLEV + (JFLD -1) * KFLEVG
    ...
    ! copy u and v
    IF (LDACC) THEN
      !$ACC KERNELS PRESENT(ZPGPUV,YYLGVU(:)%VIEW%P,LGVV(:)%VIEW%P) COPY(IUVG, KFLEVG,IOFFSET)
      DO JFLD=1,IUVG
        DO JLEV=1,KFLEVG
          ID = JLEV + (JFLD -1) * KFLEVG
    ...
    IF (LLUVDER) THEN
      IF (LDACC) THEN
        !$ACC KERNELS PRESENT(ZPGPUV,YLGVU_NS(:)%VIEW%P,YLGVV_NS(:)%VIEW%P) COPY(IUVG, KFLEVG,IOFFSET)
        DO JFLD=1,IUVG
          DO JLEV=1,KFLEVG
            ID = JLEV + (JFLD -1) * KFLEVG
...
IF (IFLDXG > 0) THEN
  ...
  IF (LDACC) THEN
    !$ACC KERNELS PRESENT(ZPGP2,YLGVSCALAR(:)%VIEW%P) COPY(IFLDXG)
    DO JFLD=1, IFLDXG
  ...
  IF (LLSCDERS) THEN
    IF (LDACC) THEN
      !$ACC KERNELS PRESENT(ZPGP2,YLGVSCALAR_NS(:)%VIEW%P,YLGVSCALAR_EW(:)%VIEW%P) COPY(IFLDXG)
      DO JFLD=1,IFLDXG
