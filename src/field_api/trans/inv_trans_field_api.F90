  ...
  ENDIF
    ...
    DO JFLD=1,IFLDSPVOR
      IF (YLSPVVOR(JFLD)%IVSET == KPROC) THEN
        ...
        IF (LDACC) THEN
          CALL LEGACY_1RB_ASSIGN(ZPSPVOR(JFLD,:),ZZ1_1)
          CALL LEGACY_1RB_ASSIGN(ZPSPDIV(JFLD,:),ZZ1_2)          
        ELSE
          ZPSPVOR(JFLD,:) = ZZ1_1(:)
          ZPSPDIV(JFLD,:) = ZZ1_2(:)
        ENDIF
...
! allocate 'b-set' for scalar fields
  ...
  DO JFLD = 1,IFLDSPSC
    IF (YLSPVSCALAR(JFLD)%IVSET == KPROC) THEN
      ...
      IF (LDACC) THEN
        CALL LEGACY_1RB_ASSIGN(ZZPSPSC2(ID,:),ZZ1_1)
      ELSE
        ZPSPSC2(ID,:) = ZZ1_1(:)
      ENDIF
      ID = ID + 1
...
IF (IUVG>0) THEN
  ...
  IF (LLVORGP) THEN
      DO JFLD=1,IUVG
        DO JLEV=1,KFLEVG
          ...
          IF (LDACC) THEN
            CALL LEGACY_2RB_ASSIGN(ZZ2_1,ZPGPUV(:, JLEV,JFLD+IOFFSET*IUVG,:))            
          ELSE
  ...
  DO JFLD=1,IUVG
    DO JLEV=1,KFLEVG
      ...
      IF (LDACC) THEN\
        CALL LEGACY_2RB_ASSIGN(ZZ2_1,ZPGPUV(:,JLEV,JFLD+IOFFSET*IUVG,:))
        CALL LEGACY_2RB_ASSIGN(ZZ2_2,ZPGPUV(:,JLEV,JFLD+(IOFFSET+1)*IUVG,:))        
      ELSE
        ZZ2_1(:,:) =  ZPGPUV(:,JLEV,JFLD+IOFFSET*IUVG,:)
  ...
  IF (LLUVDER) THEN
    DO JFLD=1,IUVG
      DO JLEV=1,KFLEVG
        ...
        IF (LDACC) THEN
          CALL LEGACY_2RB_ASSIGN(ZZ2_1,ZPGPUV(:,JLEV,JFLD+IUVG*IOFFSET,:))
          CALL LEGACY_2RB_ASSIGN(ZZ2_2, ZPGPUV(:,JLEV,JFLD+IUVG*(IOFFSET+1),:))          
        ELSE
...
IF (IFLDXG > 0) THEN
  ! copy spectral scalar fields
    ...
    DO JFLD=1, IFLDXG
      ...
      IF (LDACC) THEN
        CALL LEGACY_2RB_ASSIGN(ZZ2_1,ZPGP2(:,JFLD,:))        
      ELSE
        ZZ2_1(:,:) = ZPGP2(:,JFLD,:)
      ENDIF
    ENDDO
  ...
  IF (LLSCDERS) THEN
      DO JFLD=1,IFLDXG
        ...
        IF (LDACC) THEN
          CALL LEGACY_2RB_ASSIGN(ZZ2_1,ZPGP2(:, JFLD+IFLDXG,:))
          CALL LEGACY_2RB_ASSIGN(ZZ2_1,ZPGP2(:, JFLD+(2*IFLDXG),:)          
        ELSE
