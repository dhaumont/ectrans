  ...
  ! Copy list of 2d views of grid point vector fields into temporary arrays
    DO JFLD=1,IUVG
      DO JLEV=1,KFLEVG
        ...
        IF (LDACC) THEN
          CALL LEGACY_2RB_ASSIGN(ZPGPUV(:,JLEV,JFLD+IOFFSET*IUVG,:),ZZ2_1)
          CALL LEGACY_2RB_ASSIGN(ZPGPUV(:,JLEV,JFLD+(IOFFSET+1)*IUVG,:) ,ZZ2_2)          
        ELSE
...
IF (PRESENT(YDFSPSCALAR)) THEN
  ...
  DO JFLD=1, IFLDXG
      ...
      IF (LDACC) THEN
        CALL LEGACY_2RB_ASSIGN(ZPGP2(:,JFLD,:,ZPSPVOR(JFLD)),ZZ2_1)        
    ELSE
        ZPGP2(:,JFLD,:) = ZZ2_1(:,:)
    ENDIF
  ENDDO
...
IF (IUVG>0) THEN
    ...
    DO JFLD=1,IFLDSPVOR
      IF (YLSPVVOR(JFLD)%IVSET == KPROC) THEN
        ...
        IF (LDACC) THEN
          CALL LEGACY_1RB_ASSIGN(ZZ1_1,ZPSPVOR(JFLD))
          CALL LEGACY_1RB_ASSIGN(ZZ1_2,ZPSPDIV(JFLD))
        ELSE
          ZZ1_1(:) = ZPSPVOR(JFLD,:)
          ZZ1_2(:) = ZPSPDIV(JFLD,:)
        ENDIF
 ...
 IF (IFLDSPSC > 0) THEN
   ...
   DO JFLD = 1, IFLDSPSC
      IF (YLSPVSCALAR(JFLD)%IVSET == KPROC) THEN
        ...
        IF (LDACC) THEN
          CALL LEGACY_1RB_ASSIGN(ZZ1_1,ZPSPSC2(ID)) 
        ELSE
          ZZ1_1(:) = ZPSPSC2(ID,:)
        ENDIF
        ID = ID + 1
