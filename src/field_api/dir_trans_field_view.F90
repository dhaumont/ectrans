...
IF (SIZE(YDGPU) > 0) THEN
  ...
  DO JFLD=1,IUVG
    ...
    IF (SIZE(IVSET_PAIRED) /= KFLEVG) &
      & CALL ABOR1("[DIR_TRANS_FIELD_API] YDSPVOR ivset size inconsistent with KFLEVG")
    DO JLEV=1,KFLEVG
      ID = JLEV + (JFLD - 1) * KFLEVG
  ...
  DO JFLD=1,IUVG
    DO JLEV=1,KFLEVG
      ...
      IF (IVSETUV(JLEV) .NE. YLGVV(ID)%IVSET) &
        & CALL ABOR1("[DIR_TRANS_FIELD_API] ivsetuv inconsistent with ylgvv%ivset")
    ENDDO
  ENDDO
ELSE
...
IF (SIZE(YDSPSCALAR) > 0 ) THEN
  ...
  DO JFLD = 1, SIZE(YDSPSCALAR)
    ...
    DO JLEV = 1, SIZE(IVSET_PAIRED)
      ID = ID + 1
      IF (ID > IFLDXG) &
        & CALL ABOR1("[DIR_TRANS_FIELD_API] more ivset entries than grid scalar views")
      YLGVSCALAR(ID)%IVSET = IVSET_PAIRED(JLEV)
    ENDDO
  ENDDO
  IF (ID /= IFLDXG) &
    & CALL ABOR1("[DIR_TRANS_FIELD_API] ivset count mismatch between YDSPSCALAR and YDGPSCALAR")
