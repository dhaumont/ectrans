...
NPROMA = 0 
NSPEC2 = 0
WRITE(NOUT,*) "CALL INV_TRANS_FIELD_API BEGIN"

CALL GET_LAYOUT_G(YDFU, NBLK, NPROMA)
CALL GET_LAYOUT_G(YDFV, NBLK, NPROMA)
CALL GET_LAYOUT_G(YDFSCALAR,NBLK, NPROMA)
CALL GET_LAYOUT_S(YDFSPVOR, NSPEC2)
CALL GET_LAYOUT_S(YDFSPDIV, NSPEC2)
CALL GET_LAYOUT_S(YDFSPSCALAR, NSPEC2)
CALL GET_LAYOUT_S(YDFSPVOR, NSPEC2)
...
IF (PRESENT(YDFSPSCALAR)) THEN
 ...
 ENDIF
  WRITE(NOUT,*) "IFLDXL", IFLDXL
  WRITE(NOUT,*) "IFLDSPSC", IFLDSPSC
...
WRITE(NOUT,*) "INV_TRANS_FIELD_API: call inv_trans begin"
...
IF (IUVG>0) THEN
  ...
  ! copy vorticity
  IF (LLVORGP) THEN
    WRITE(NOUT,*) "INV_TRANS_FIELD_API: copy vorticity"
  ...
  ! copy divergence
  IF (LLDIVGP) THEN
    WRITE(NOUT,*) "INV_TRANS_FIELD_API: copy divergence"
  ...
  ! copy u and v
    WRITE(NOUT,*) "INV_TRANS_FIELD_API: copy uv"
  ...
  IF (LLUVDER) THEN
    WRITE(NOUT,*) "INV_TRANS_FIELD_API: copy uv derivatives"
...
IF (IFLDXG > 0) THEN
    WRITE(NOUT,*) "INV_TRANS_FIELD_API: copy spectral scalar fields "
  ...
  IF (LLSCDERS) THEN
    WRITE(NOUT,*) "INV_TRANS_FIELD_API: copy spectral scalar fields derivatives"
...
! 5. Final cleanup
WRITE(NOUT,*) "INV_TRANS_FIELD_API: begin final cleanup"
...
WRITE(NOUT,*) "CALL INV_TRANS_FIELD_API END"
