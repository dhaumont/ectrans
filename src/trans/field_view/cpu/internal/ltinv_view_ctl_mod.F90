...
USE YOMGSTATS, ONLY: NCALLS, CCDESC
...
! DEBUG: print state before and after GSTATS(102, 0) to diagnose stats issue
WRITE(0,'(A,I0,A,Z16,A,A)') '[LTINV_VIEW_CTL] BEFORE GSTATS(102,0): NCALLS(102)=', &
  & NCALLS(102), ' addr(NCALLS(102))=0x', LOC(NCALLS(102)), ' CCDESC(102)="', TRIM(CCDESC(102))//'"'
CALL GSTATS(102,0)
WRITE(0,'(A,I0,A,Z16,A,A)') '[LTINV_VIEW_CTL] AFTER  GSTATS(102,0): NCALLS(102)=', &
  & NCALLS(102), ' addr(NCALLS(102))=0x', LOC(NCALLS(102)), ' CCDESC(102)="', TRIM(CCDESC(102))//'"'
