...
CONTAINS
SUBROUTINE UPDSP_VIEW(POA1, POA2, YDSPVVOR, YDSPVDIV, YDSPVSCALAR)
...
!        YDSPVVOR - vorticity field
!        YDSPVDIV - divergence field
...
!     LOCAL INTEGER SCALARS
INTEGER(KIND=JPIM) ::  IVORS, IVORE, IDIVS, IDIVE, JFLD
...
IF (IF_UV > 0) THEN
  ...
  IDIVS = 2*IF_UV+1
  IDIVE = 4*IF_UV
  CALL UPDSPB_VIEW(POA2(IVORS:IVORE,:,:),YDSPVVOR)
  CALL UPDSPB_VIEW(POA2(IDIVS:IDIVE,:,:),YDSPVDIV)
  !IF (KM == 0) THEN
   ! DO JFLD=1,IF_UV
