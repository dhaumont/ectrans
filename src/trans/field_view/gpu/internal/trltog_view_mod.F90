...
#define TRLTOG_DTYPE MPI_REAL8
    ...
    !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,ZZ) FIRSTPRIVATE(KF_FS)
      DO JFLD=1,KF_FS    
         IFLD = IFLDA(JFLD,1)
         ZZ=>YDGP(IFLD)%P 
       
         !$ACC PARALLEL LOOP PRIVATE(JK,JBLK,IPOS)DEVICEPTR(ZZ) FIRSTPRIVATE(KF_FS,IRECV_WSET_SIZE_V,IRECV_WSET_OFFSET_V, &
    ...
    DO INR=1,IRECV_COUNTS
      ...
      !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,IFLD,ZZ) FIRSTPRIVATE(IRECV_FIELD_COUNT_V,IFLDA,INR)
    
      DO JFLD=1,IRECV_FIELD_COUNT_V      
         IFLD=IFLDA(JFLD,1+INR)
          ZZ=>YDGP(IFLD)%P
  
      !$ACC PARALLEL LOOP PRIVATE(JK,JBLK,JI)DEVICEPTR(ZZ) FIRSTPRIVATE(IRECV_FIELD_COUNT_V,INR,IRECV_WSET_SIZE_V,&
