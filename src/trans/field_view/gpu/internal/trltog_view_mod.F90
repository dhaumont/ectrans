...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IFLD,ZZ)FIRSTPRIVATE(KF_FS) ASYNC(1)
#endif
      DO JFLD=1,KF_FS    
         IFLD = IFLDA(JFLD,1)
          ZZ=>YDGP(IFLD)%P 
         !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,IPOSZ) &
         !$ACC&         FIRSTPRIVATE(IRECV_WSET_SIZE_V,IRECV_WSET_OFFSET_V, &
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JI,ZZ) &
      !$ACC&   FIRSTPRIVATE(IRECV_FIELD_COUNT_V) ASYNC(1)
#endif
      DO JFLD=1,IRECV_FIELD_COUNT_V      
         IFLD=IFLDA(JFLD,1+INR)
          ZZ=>YDGP(IFLD)%P
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLKLD,JI) &
      !$ACC&              FIRSTPRIVATE(INR,IRECV_WSET_SIZE_V,&
