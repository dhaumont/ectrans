...
#define TRLTOG_DTYPE MPI_REAL8
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP GANG DEFAULT(NONE) PRIVATE(IFLD,ZZ) FIRSTPRIVATE(KF_FS)
#endif
      DO JFLD=1,KF_FS    
         IFLD = IFLDA(JFLD,1)
          ZZ=>YDGP(IFLD)%P 
         !$ACC PARALLEL LOOP VECTOR PRIVATE(JK,JBLK,IPOSZ) &
         !$ACC&         FIRSTPRIVATE(IRECV_WSET_SIZE_V,IRECV_WSET_OFFSET_V, &
         !$ACC&         IIN_TO_SEND_BUFR_V,NPROMA) 
    ...
    DO INR=1,IRECV_COUNTS
      ...
      !$ACC&   FIRSTPRIVATE(IRECV_FIELD_COUNT_V) 
#endif
      DO JFLD=1,IRECV_FIELD_COUNT_V      
         IFLD=IFLDA(JFLD,1+INR)
          ZZ=>YDGP(IFLD)%P
      !$ACC PARALLEL LOOP VECTOR PRIVATE(JK,JBLKLD,JI) &
