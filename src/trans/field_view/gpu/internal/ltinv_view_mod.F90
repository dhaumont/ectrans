...
MODULE LTINV_VIEW_MOD
  ...
  PRIVATE
  PUBLIC :: LTINV_VIEW, LTINV_HANDLE, PREPARE_LTINV
...
CONTAINS
  ...
  SUBROUTINE LTINV_VIEW(ALLOCATOR,HLTINV,KF_UV,KF_SCALARS,&
    ...
    IF (KF_SCALARS > 0) THEN
      ...
      ELSE
        ...
        IF(IFIRST-1 /= 2*KF_SCALARS) THEN
          WRITE(0,*) 'LTINV_VIEW:KF_SCALARS,IFIRST',KF_SCALARS,IFIRST
          CALL ABORT_TRANS('LTINV_VIEW_MOD:IFIRST /= 2*KF_SCALARS')
        ENDIF
      ENDIF
    ENDIF
