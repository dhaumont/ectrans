...
MODULE TRGTOL_VIEW_MOD
  USE BUFFERED_ALLOCATOR_MOD, ONLY: ALLOCATION_RESERVATION_HANDLE
  IMPLICIT NONE

  PRIVATE
  PUBLIC :: TRGTOL_HANDLE, TRGTOL_VIEW, PREPARE_TRGTOL
...
CONTAINS
  ...
  SUBROUTINE TRGTOL_VIEW(ALLOCATOR,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KVSET, YDGP)

    !**** *TRGTOL_VIEW * - transposition of grid point data from column
    ...
    !           YDGP    -  Blocked grid point data    (input)
...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    IF (LHOOK) CALL DR_HOOK('TRGTOL_VIEW',0,ZHOOK_HANDLE)
    ...
    DO INS=1,ISEND_COUNTS
      ...
      !$OMP& ISEND_WSET_OFFSET_V,INS,ICOMBUFS_OFFSET_V,IFLDA,ZCOMBUFS) &
      !$OMP& MAP(TO:ISEND_FIELD_COUNT_V,ISEND_WSET_SIZE_V,NPROMA,ISEND_WSET_OFFSET_V,&
      !$OMP& INS,ICOMBUFS_OFFSET_V)
#endif

      !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,IFLD,ZZ) FIRSTPRIVATE(ISEND_FIELD_COUNT_V,IFLDA,INS)
      DO JFLD=1,ISEND_FIELD_COUNT_V   
        IFLD = IFLDA(JFLD,1+INS)
        ZZ=>YDGP(IFLD)%P   
       
        !$ACC  PARALLEL LOOP PRIVATE(JK,JBLK,JI) DEVICEPTR(ZZ) FIRSTPRIVATE(ISEND_FIELD_COUNT_V,INS,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,&
        ...
        DO JL=1,ISEND_WSET_SIZE_V       
          ...
          ZCOMBUFS(ICOMBUFS_OFFSET_V+JI) = ZZ(JK,JBLK)
        ENDDO
    ...
    IF(LLOCAL_CONTRIBUTION)THEN
      ...
      !$OMP& IRECV_BUFR_TO_OUT_V,IRECV_BUFR_TO_OUT,PREEL_REAL) MAP(TO:KF_FS,IRECV_BUFR_TO_OUT_V)
#endif
 
    !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,IFLD,ZZ) FIRSTPRIVATE(KF_FS,IFLDA)
    
      DO JFLD=1,KF_FS
        IFLD = IFLDA(JFLD,1)
        ZZ=>YDGP(IFLD)%P
      
      !$ACC PARALLEL LOOP PRIVATE(JK,JBLK,IPOS) DEVICEPTR(ZZ)  FIRSTPRIVATE(KF_FS,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,&
    ...
    IF(IR > 0) THEN
      CALL MPL_WAIT(KREQUEST=IREQ(1:IR), &
        & CDSTRING='TRGTOL_VIEW: WAIT FOR SENDS AND RECEIVES')
    ENDIF
#ifdef USE_GPU_AWARE_MPI
#ifdef ACCGPU
    !$ACC END HOST_DATA
    ...
    IF (LHOOK) CALL DR_HOOK('TRGTOL_VIEW',1,ZHOOK_HANDLE)

  END SUBROUTINE TRGTOL_VIEW
END MODULE TRGTOL_VIEW_MOD
