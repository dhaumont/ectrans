...
#endif
#ifdef ACCGPU
    !$ACC DATA COPYIN(YDGP,IFLDA) ASYNC(1) 
#endif
...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP GANG DEFAULT(NONE) PRIVATE(IFLD,ZZ) &
      !$ACC&              FIRSTPRIVATE(ISEND_FIELD_COUNT_V) 
#endif
      DO JFLD=1,ISEND_FIELD_COUNT_V   
        IFLD = IFLDA(JFLD,1+INS)
        ZZ=>YDGP(IFLD)%P   
        !$ACC PARALLEL LOOP VECTOR PRIVATE(JK,JBLK,JI) &
        !$ACC&              FIRSTPRIVATE(INS,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,&
        !$ACC&              ICOMBUFS_OFFSET_V,NPROMA) 
...
#ifdef ACCGPU
      !$ACC PARALLEL LOOP GANG DEFAULT(NONE) PRIVATE(IFLD,ZZ) &
      !$ACC&              FIRSTPRIVATE(KF_FS) 
#endif
      DO JFLD=1,KF_FS
        IFLD = IFLDA(JFLD,1)
        ZZ=>YDGP(IFLD)%P
         
      !$ACC PARALLEL LOOP VECTOR PRIVATE(JK,JBLK,IPOS) &
      !$ACC&              FIRSTPRIVATE(ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,&
      !$ACC&              IRECV_BUFR_TO_OUT_V,NPROMA) 
        DO JL=1,ISEND_WSET_SIZE_V
          JK = MOD(ISEND_WSET_OFFSET_V+JL-1,NPROMA)+1
          JBLK = (ISEND_WSET_OFFSET_V+JL-1)/NPROMA+1
