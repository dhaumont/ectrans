...
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IFLD,ZZ) &
      !$ACC&              FIRSTPRIVATE(ISEND_FIELD_COUNT_V) ASYNC(1)
#endif
      DO JFLD=1,ISEND_FIELD_COUNT_V   
        IFLD = IFLDA(JFLD,1+INS)
        ZZ=>YDGP(IFLD)%P   
        !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(JK,JBLK,JI) &
...
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IFLD,ZZ) &
      !$ACC&              FIRSTPRIVATE(KF_FS) ASYNC(1)
#endif
      DO JFLD=1,KF_FS
        IFLD = IFLDA(JFLD,1)
        ZZ=>YDGP(IFLD)%P
         
      !$ACC PARALLEL LOOP  DEFAULT(NONE) PRIVATE(JK,JBLK,IPOS) &
