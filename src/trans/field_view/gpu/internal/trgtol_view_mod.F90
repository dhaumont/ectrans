...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    DO INS=1,ISEND_COUNTS
      ...
      DO JFLD=1,ISEND_FIELD_COUNT_V   
        ...
        ZZ=>YDGP(IFLD)%P   
        !$ACC HOST_DATA USE_DEVICE(ZZ)
        !$ACC  PARALLEL LOOP PRIVATE(JK,JBLK,JI) FIRSTPRIVATE(ISEND_FIELD_COUNT_V,INS,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,&
    ...
    IF(LLOCAL_CONTRIBUTION)THEN
      ...
      DO JFLD=1,KF_FS
        ...
        ZZ=>YDGP(IFLD)%P
         !$ACC HOST_DATA USE_DEVICE(ZZ)
      !$ACC PARALLEL LOOP PRIVATE(JK,JBLK,IPOS)   FIRSTPRIVATE(KF_FS,ISEND_WSET_SIZE_V,ISEND_WSET_OFFSET_V,&
