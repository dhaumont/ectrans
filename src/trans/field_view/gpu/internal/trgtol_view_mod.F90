...
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(TO:IFLDA) PRESENT(YDGP)
#endif
#ifdef ACCGPU
    !$ACC DATA COPYIN(IFLDA) ASYNC(1) PRESENT(YDGP)
#endif
    ...
    DO INS=1,ISEND_COUNTS
      ...
      DO JFLD=1,ISEND_FIELD_COUNT_V
        DO JL=1,ISEND_WSET_SIZE_V
          ...
          ZCOMBUFS(ICOMBUFS_OFFSET_V+JI) = YDGP(IFLD)%P(JK,JBLK)
        ENDDO
      ENDDO
    ...
    IF(LLOCAL_CONTRIBUTION)THEN
      ...
      DO JFLD=1,KF_FS
        DO JL=1,ISEND_WSET_SIZE_V
          ...
          PREEL_REAL(IPOS) = YDGP(IFLD)%P(JK,JBLK)
        ENDDO
      ENDDO
