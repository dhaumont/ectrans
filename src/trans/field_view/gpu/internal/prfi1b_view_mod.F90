...
IFIELDS = SIZE(YDSP)
  ...
  !!OMP TARGET DATA MAP(PRESENT,ALLOC:D,D_NUMP,R,R_NSMAX,D_MYMS,D_NASM0,PIA,PSPEC)
    ...
    WRITE(NOUT,*)"PRFI1B: LBOUND(PIA)", LBOUND(PIA,1), LBOUND(PIA,2), LBOUND(PIA,3)
    WRITE(NOUT,*)"PRFI1B: UBOUND(PIA)", UBOUND(PIA,1), UBOUND(PIA,2), UBOUND(PIA,3)
    !loop over wavenumber
    WRITE(NOUT,*) "D_NUMP", D_NUMP
    WRITE(NOUT,*) "R_NSMAX", R_NSMAX
    WRITE(NOUT,*) "D_MYMS", D_MYMS
    WRITE(NOUT,*) "D_NASM0", D_NASM0
#ifdef OMPGPU
