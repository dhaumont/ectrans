...
IFIELDS = SIZE(YDSP)
#ifdef ACCGPU
  !$ACC DATA PRESENT(D,D_NUMP,R,R_NSMAX,D_MYMS,D_NASM0,PIA) ASYNC(1)
#endif
#ifdef OMPGPU
...
#endif
  !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,ZZ) FIRSTPRIVATE(IFIELDS)
    DO JFLD=1,IFIELDS  
       ZZ=>YDSP(JFLD)%P

  !$ACC PARALLEL LOOP COLLAPSE(2) DEVICEPTR(ZZ) PRIVATE(KM,IASM0,INM,KM,IASM0,INM)  ASYNC(1)
