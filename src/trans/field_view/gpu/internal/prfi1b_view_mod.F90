...
WRITE(NOUT,*)"PRFI1B: PIA", SHAPE(PIA), "YDSP", SHAPE(YDSP), "IFIELDS:", IFIELDS
    !loop over wavenumber
    WRITE(NOUT,*) D_NUMP, R_NSMAX, D_MYMS, D_NASM0
#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) &
    !$OMP& PRIVATE(KM,IASM0,INM) SHARED(IFIELDS,D,R,PIA,PSPEC) MAP(TO:IFIELDS)
#endif
#ifdef ACCGPU
  ...
  DO KMLOC=1,D_NUMP
    DO JN=0,R_NSMAX+3
      DO JFLD=1,IFIELDS
        ...
        IF (JN <= 1) THEN
            PIA(1+2*JFLD-1,JN+1,KMLOC) = 0.0_JPRB
            PIA(1+2*JFLD  ,JN+1,KMLOC) = 0.0_JPRB
        ELSEIF (JN <= R_NSMAX+2-KM) THEN
            IASM0 = D_NASM0(KM)
            INM = IASM0+((R_NSMAX+2-JN)-KM)*2
            PIA(1+2*JFLD-1,JN+1,KMLOC) = YDSP(JFLD)%P(INM)
            PIA(1+2*JFLD  ,JN+1,KMLOC) = YDSP(JFLD)%P(INM+1)
