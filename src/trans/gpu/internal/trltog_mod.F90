...
#endif
#ifdef OMPGPU
!WARNING: following ALLOC statement should be PRESENT,ALLOC but causes issues with AMD compiler!
  !$OMP TARGET DATA &
  !$OMP& MAP(ALLOC:PGLAT) &
  ...
  IF( IRECVTOT(MYPROC) > 0 )THEN
    ...
    IF (LLPGPONLY) THEN
#ifdef OMPGPU
      !$OMP TARGET PARALLEL DO DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
  ...
  CALL GSTATS(1605,0)
    ...
    DO INS=1,INSEND
...
#ifdef OMPGPU
      !$OMP TARGET PARALLEL DO DEFAULT(NONE) PRIVATE(II) MAP(TO:ILEN) COLLAPSE(2) &
	  ...
	  !$ACC&  COPYIN(ILEN,INS,KF_FS,ISEND)
#endif
      DO JL=1,ILEN
        DO JFLD=1,KF_FS
...
#ifdef OMPGPU
      !$OMP TARGET PARALLEL DO DEFAULT(NONE) PRIVATE(IFIRST,ILAST,JI,JK,IFLDT) &
