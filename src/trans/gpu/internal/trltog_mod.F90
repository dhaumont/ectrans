...
MODULE TRLTOG_MOD
 ...
 &                            PGP,PGPUV,PGP3A,PGP3B,PGP2,LDTRANSPOSED)
  ...
  LOGICAL, OPTIONAL, INTENT(IN) :: LDTRANSPOSED   ! key indicating whether PGLAT is transposed, i.e. (nlon*nlat,nfld) instead of (nfld,nlon*nlat)
...
#define TRLTOG_DTYPE MPI_DOUBLE_PRECISION
  ...
  IF (PRESENT(LDTRANSPOSED)) LLTRANSPOSED=LDTRANSPOSED
  ...
  !$ACC KERNELS DEFAULT(NONE) COPYIN(IBUFLENS,IBUFLENR)
#endif
  ...
  IF( IRECVTOT(MYPROC) > 0 )THEN
    ...
    IF (LLPGPONLY) THEN
      ...
      !$ACC&     COPYIN(NGPBLKS,IFLDS,IGPTRSEND,MYSETW,INDOFF,MYPROC, &
      !$ACC&            LLINDER,KPTRGP,KINDEX,LLTRANSPOSED)
	  !$ACC&     PRESENT(PGP,PGLAT)
#endif
      DO JBLK=1,NGPBLKS
        DO JFLD=1,IFLDS
          DO JKL=1,JK_MAX
            ...
            IF(IFIRST>0 .AND. JK<=ILAST) THEN
              ...
              IF(LLINDER) THEN
                IFLD = KPTRGP(JFLD)
                IF (LLTRANSPOSED) THEN
				  PGP(JK,IFLD,JBLK) = PGLAT(KINDEX(IPOS),JFLD)
				ELSE
    ...
    ELSE
      ...
      !$ACC&     COPYIN(NGPBLKS,IFLDS,IGPTRSEND,MYSETW,INDOFF,MYPROC, &
      !$ACC&            LLUV,PGPUV,IUVLEVS,IUVPARS,LLGP2,PGP2,IGP2PARS, &
      !$ACC&            LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS,LLGP3B,PGP3B,&
      !$ACC&            IGP3BLEVS,IGP3BPARS,KINDEX,LLTRANSPOSED)
      !$ACC&     PRESENT(PGLAT,PGPUV,PGP2,PGP3A,PGP3B)
#endif
      DO JBLK=1,NGPBLKS
        DO JFLD=1,IFLDS
          DO JKL=1,JK_MAX
  ...
  CALL GSTATS(1605,0)
    ...
    DO INS=1,INSEND
	  ...
	  !$ACC&  COPYIN(ILEN,INS,KF_FS,ISEND,LLTRANSPOSED)
#endif
      DO JL=1,ILEN
        DO JFLD=1,KF_FS
...
!      IRECV_FLD_START = 1 !! INT(ZCOMBUFR(-1,INR),KIND=JPIM) !! is this always 1 ?
      ...
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IFIRST,ILAST,JI,JK,IFLDT) &
      !$ACC&     COPYIN(INR,KF_FS,IPOS,JPOS,IFLD,IFLDA,JK_MAX,IRECV_FLD_END) COLLAPSE(3) &
      !$ACC&     COPYIN(NGPBLKS,IRECV_FLD_END,JK_MAX,IFLDA,IGPTRSEND,ISETW,IPOS,JPOS, &
      !$ACC&            LLINDER,KPTRGP,INR,LLPGPONLY,LLUV,IUVLEVS, &
      !$ACC&            IUVPARS,LLGP2,IGP2PARS,LLGP3A,IGP3ALEVS,IGP3APARS, &
      !$ACC&            LLGP3B,IGP3BLEVS,IGP3BPARS) &
	  !$ACC&     PRESENT(PGP,ZCOMBUFR,PGPUV,PGP2,PGP3A,PGP3B)
#endif
      DO JBLK=1,NGPBLKS
