...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  IF(LLPGPONLY) THEN
    ...
    !$ACC& COPYIN(NGPBLKS,IFLDS,JK_MAX,IGPTRSEND,MYSETW,INDOFF,MYPROC,IGPTROFF,IFLDOFF,KINDEX,LLTRANSPOSED) &
	!$ACC& PRESENT(PGLAT,PGP)
#endif
    DO JBLK=1,NGPBLKS
      DO JFLD=1,IFLDS
        DO JKL=1, JK_MAX
...
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
    !$ACC& COPYIN(NGPBLKS,IFLDS,JK_MAX,IGPTRSEND,MYSETW,INDOFF,MYPROC,IGPTROFF,IFLDOFF, &
    !$ACC&        LLUV,LLGP2,LLGP3A,LLGP3B,KINDEX,IUVLEVS, &
    !$ACC&        IUVPARS,IGP2PARS,IGP3ALEVS,IGP3APARS,IGP3BLEVS,IGP3BPARS,LLTRANSPOSED) &
...
#ifdef ACCGPU
      !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFLDT,IFIRST,ILAST) COPYIN(INS,JK_MAX,IJPOS,IFLDA) &
      !$ACC& COPYIN(ISEND_FLD_END,NGPBLKS,IFLDA,ISETW,IPOS,JK_MAX,IGPTRSEND,IJPOS,LLINDER,LLPGPONLY, &
      !$ACC&        LLUV,LLGP2,LLGP3A,LLGP3B,INS,KPTRGP,IUVLEVS, &
      !$ACC&        IUVPARS,IGP2PARS,IGP3ALEVS,IGP3APARS,IGP3BLEVS,IGP3BPARS) &
	  !$ACC& PRESENT(ZCOMBUFS,PGP,PGPUV,PGP2,PGP3A,PGP3B)
#endif
  ...
  DO INR=1,INRECV
      ...
      !$ACC& COPYIN(IRECV_FLD_END,ILEN,IRECV,INR,LLTRANSPOSED)
	  !$ACC& PRESENT(KINDEX,ZCOMBUFR,PGLAT)
