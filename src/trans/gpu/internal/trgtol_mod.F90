...
!WARNING: following lines should be PRESENT,ALLOC but cause issues with AMD compiler!
  ...
  IF(LLPGPONLY) THEN
#ifdef OMPGPU
    !$OMP TARGET PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
    ...
    !$ACC& COPYIN(NGPBLKS,IFLDS,JK_MAX,IGPTRSEND,MYSETW,INDOFF,MYPROC,IGPTROFF,IFLDOFF,KINDEX) &
...
#ifdef OMPGPU
      !$OMP TARGET PARALLEL DO COLLAPSE(3) DEFAULT(NONE) PRIVATE(JK,JI,IFLDT,IFIRST,ILAST) &
  ...
  DO INR=1,INRECV
...
#ifdef OMPGPU
      !$OMP TARGET PARALLEL DO COLLAPSE(2) DEFAULT(NONE) PRIVATE(II) &
      ...
      !$ACC& COPYIN(IRECV_FLD_END,ILEN,IRECV,INR)
