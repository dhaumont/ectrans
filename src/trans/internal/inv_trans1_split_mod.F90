...
INTEGER (KIND=JPIM) :: J
CHARACTER*16 :: CLSPLIT
INTEGER (KIND=JPIM) :: ISPLIT
INTEGER (KIND=JPIM), ALLOCATABLE :: IORD_U (:)
INTEGER (KIND=JPIM), ALLOCATABLE :: IORD_S (:)

REAL(KIND=JPHOOK) :: ZHOOK_HANDLE

IF (LHOOK) CALL DR_HOOK('INV_TRANS1_SPLIT',0,ZHOOK_HANDLE)

CALL GETENV ('SPLIT', CLSPLIT)
READ (CLSPLIT, *) ISPLIT

WRITE (0, *) __FILE__, ':', __LINE__, " ISPLIT = ", ISPLIT
...
IORD_U = SORT (YLGP_U%IVSET)

IF (SIZE (YLGP_VOR ) > 0) YLGP_VOR  = YLGP_VOR  (IORD_U)
IF (SIZE (YLGP_DIV ) > 0) YLGP_DIV  = YLGP_DIV  (IORD_U)
IF (SIZE (YLGP_U   ) > 0) YLGP_U    = YLGP_U    (IORD_U)
IF (SIZE (YLGP_V   ) > 0) YLGP_V    = YLGP_V    (IORD_U)
IF (SIZE (YLGP_U_EW) > 0) YLGP_U_EW = YLGP_U_EW (IORD_U)
IF (SIZE (YLGP_V_EW) > 0) YLGP_V_EW = YLGP_V_EW (IORD_U) 

IORD_S = SORT (YLGP_S%IVSET)

IF (SIZE (YLGP_S   ) > 0) YLGP_S    = YLGP_S    (IORD_S)
IF (SIZE (YLGP_S_NS) > 0) YLGP_S_NS = YLGP_S_NS (IORD_S)
IF (SIZE (YLGP_S_EW) > 0) YLGP_S_EW = YLGP_S_EW (IORD_S)
...
FUNCTION SORT (KVSET) RESULT (IORD)

INTEGER (KIND=JPIM), ALLOCATABLE :: IORD (:)
INTEGER (KIND=JPIM) :: KVSET (:)
LOGICAL :: LLSEEN (SIZE (KVSET))

INTEGER (KIND=JPIM) :: I, J, IVSET

LLSEEN = .FALSE.

ALLOCATE (IORD (SIZE (KVSET)))

I = 0

DO WHILE (I < SIZE (IORD))

  IVSET = 0

  DO J = 1, SIZE (KVSET)
    IF ((.NOT. LLSEEN (J)) .AND. (KVSET (J) > IVSET)) THEN
      IVSET = KVSET (J)
      I = I + 1
      IORD (I) = J
      LLSEEN (J) = .TRUE.
