...
SUBROUTINE TRLTOG_VIEW(PGLAT, KF_FS, KF_GP, KVSET, YDGP,KPTRGP)
...
CALL TRLTOG_VIEW_COMM(PGLAT, KF_FS, KF_GP, KVSET, KPTRGP, YDGP, YDBUFS)
...
SUBROUTINE TRLTOG_VIEW_COMM(PGLAT, KF_FS, KF_GP, KVSET, KPTRGP, YDGP,YDBUFS)
...
USE TRGL_MOD, ONLY: TRGL_BUFFERS, TRGL_VARS, TRGL_ALLOCATE_VARS, TRGL_ALLOCATE_HEAP_BUFFER, &
  &                 TRGL_INIT_OFF_VARS, TGRL_INIT_PACKING_VARS

USE TRGL_VIEW_MOD, ONLY: TGRL_COPY_ZCOMBUF_VIEW, TGRL_COPY_PGLAT_VIEW
...
CALL GSTATS(1806,0)
YDBUFS%LLINDER = .FALSE.
YDBUFS%LLPGPONLY = .FALSE.
...
IF(KRECVTOT(MYPROC) > 0 )THEN
  CALL TRGL_INIT_OFF_VARS(YDBUFS,YLVARS,KVSET,KF_GP,KPTRGP)
  CALL GSTATS(1604,0)
  CALL TGRL_COPY_PGLAT_VIEW(PGLAT, YDBUFS, YLVARS, YDGP)
  CALL GSTATS(1604,1)
ENDIF
...
DO JNR=1,KNRECV
  ...
  CALL TGRL_COPY_ZCOMBUF_VIEW(YDBUFS, YLVARS, INR, ZCOMBUFR, YDGP)
