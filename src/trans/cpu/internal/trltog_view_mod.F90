...
PUBLIC TRLTOG_VIEW
PRIVATE TRLTOG_VIEW_COMM

CONTAINS

SUBROUTINE TRLTOG_VIEW(PGLAT, KF_FS, KF_GP, YLGVU,YLGVV,YLGVSCALAR,KPTRGP)

!**** *TRLTOG_VIEW * - head routine for transposition of grid point data from latitudinal
...
!           IVSET    - "v-set" for each field      (input)
...
!     Modifications.
!     --------------
!        Original  : 18-Aug-2014 from TRLTOG_VIEW
...
TYPE(GRID_VIEW), INTENT(INOUT) :: YLGVU(:),YLGVV(:)
TYPE(GRID_VIEW), INTENT(INOUT) :: YLGVSCALAR(:)
...
CALL GSTATS(1806, 0)
CALL TRGL_PROLOG(KF_FS, KF_GP, IVSET, YDBUFS)
CALL GSTATS(1806, 1)
CALL ALLOCATE_BUFFERS_SR(YDBUFS, KF_GP)

CALL TRLTOG_VIEW_COMM(PGLAT, KF_FS, KF_GP, IVSET, KPTRGP, YLGVU,YLGVV,YLGVSCALAR, &
  &              YDBUFS)
...
SUBROUTINE TRLTOG_VIEW_COMM(PGLAT, KF_FS, KF_GP, IVSET, KPTRGP, YLGVU,YLGVV,YLGVSCALAR,YDBUFS)


!**** *TRLTOG_VIEW * - transposition of grid point data from latitudinal
!                 to column structure. This takes place between inverse
!                 FFT and grid point calculations.
!                 TRLTOG_VIEW_COMM is the inverse of TRGTOL
...
USE TRGL_MOD, ONLY: TRGL_BUFFERS, TRGL_VARS, TRGL_ALLOCATE_VARS, TRGL_ALLOCATE_HEAP_BUFFER, &
  &                 TRGL_INIT_VARS, TRGL_INIT_OFF_VARS, TGRL_COPY_ZCOMBUF_VIEW, TGRL_COPY_PGLAT_VIEW, &
...
INTEGER(KIND=JPIM), INTENT(IN) :: KF_FS,KF_GP
REAL(KIND=JPRB),INTENT(IN)     :: PGLAT(KF_FS,D%NLENGTF)
INTEGER(KIND=JPIM), INTENT(IN) :: IVSET(KF_GP)
INTEGER(KIND=JPIM) ,OPTIONAL, INTENT(IN) :: KPTRGP(:)

TYPE(GRID_VIEW), INTENT(OUT) :: YLGVU(:),YLGVV(:)
TYPE(GRID_VIEW), INTENT(OUT) :: YLGVSCALAR(:)
...
IF (LHOOK) CALL DR_HOOK('TRLTOG_VIEW_BAR',0,ZHOOK_HANDLE_BAR)
CALL GSTATS_BARRIER(762)
IF (LHOOK) CALL DR_HOOK('TRLTOG_VIEW_BAR',1,ZHOOK_HANDLE_BAR)
...
IF (NTRANS_SYNC_LEVEL <= 0) THEN
  ...
  DO INR=1,KNRECV
    ...
    CALL MPL_RECV(ZCOMBUFR(-1:KRECVTOT(IRECV),INR), KSOURCE=NPRCIDS(IRECV), &
      ...
      &           CDSTRING='TRLTOG_VIEW_COMM: NON-BLOCKING IRECV' )
  ENDDO
ENDIF
...
IF(KRECVTOT(MYPROC) > 0 )THEN
  CALL TRGL_INIT_OFF_VARS(YDBUFS,YLVARS,IVSET,KPTRGP,KF_GP)
  CALL GSTATS(1604,0)
  CALL TGRL_COPY_PGLAT_VIEW(PGLAT, YDBUFS, YLVARS, [YLGVU,YLGVV,YLGVSCALAR])
  CALL GSTATS(1604,1)
ENDIF
...
DO INS=1,KNSEND
  ...
  IF (NTRANS_SYNC_LEVEL <= 1) THEN
    CALL MPL_SEND(ZCOMBUFS(-1:KSENDTOT(ISEND),INS), KDEST=NPRCIDS(ISEND), &
      ...
      &           CDSTRING='TRLTOG_VIEW_COMM: NON-BLOCKING ISEND')
  ELSE
    CALL MPL_SEND(ZCOMBUFS(-1:KSENDTOT(ISEND),INS), KDEST=NPRCIDS(ISEND), &
      &           KMP_TYPE=JP_BLOCKING_BUFFERED, KTAG=ITAG, &
      &           CDSTRING='TRLTOG_VIEW_COMM: BLOCKING BUFFERED BSEND')
  ENDIF
ENDDO

!  Unpack loop.........................................................

CALL TGRL_INIT_PACKING_VARS(YDBUFS,YLVARS, IVSET, KF_GP)

DO JNR=1,KNRECV

  IF (NTRANS_SYNC_LEVEL <= 0) THEN
    CALL MPL_WAITANY(KREQUEST=IREQ_RECV(1:KNRECV), KINDEX=INR, &
      &              CDSTRING='TRLTOG_VIEW_COMM: WAIT FOR ANY RECEIVES')
  ELSE
    INR = JNR
    IRECV=KRECV(INR)
    CALL MPL_RECV(ZCOMBUFR(-1:KRECVTOT(IRECV),INR), KSOURCE=NPRCIDS(IRECV), &
          & KMP_TYPE=JP_BLOCKING_STANDARD, KTAG=ITAG, CDSTRING='TRLTOG_VIEW_COMM: BLOCKING RECV')
  ENDIF

  CALL TGRL_COPY_ZCOMBUF_VIEW(YDBUFS, YLVARS, INR, ZCOMBUFR, KPTRGP, [YLGVU,YLGVV,YLGVSCALAR])

ENDDO

IF (NTRANS_SYNC_LEVEL <= 1) THEN
  IF(KNSEND > 0) THEN
    CALL MPL_WAIT(KREQUEST=IREQ_SEND(1:KNSEND),CDSTRING='TRLTOG_VIEW_COMM: WAIT FOR ISENDS')
  ENDIF
ENDIF

IF (NTRANS_SYNC_LEVEL >= 1) THEN
  CALL MPL_BARRIER(CDSTRING='TRLTOG_VIEW_COMM: BARRIER AT END')
ENDIF
...
END SUBROUTINE TRLTOG_VIEW_COMM
