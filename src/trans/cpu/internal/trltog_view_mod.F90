...
SUBROUTINE TRLTOG_VIEW(PGLAT, KF_FS, KF_GP, KVSET, YLGV,KPTRGP)
...
CALL TRLTOG_VIEW_COMM(PGLAT, KF_FS, KF_GP, KVSET, KPTRGP, YLGV, YDBUFS)
...
SUBROUTINE TRLTOG_VIEW_COMM(PGLAT, KF_FS, KF_GP, KVSET, KPTRGP, YLGV,YDBUFS)
...
IF(KRECVTOT(MYPROC) > 0 )THEN
  ...
  CALL GSTATS(1604,0)
  CALL TGRL_COPY_PGLAT_VIEW(PGLAT, YDBUFS, YLVARS, YLGV)
  CALL GSTATS(1604,1)
ENDIF
...
DO JNR=1,KNRECV
  ...
  CALL TGRL_COPY_ZCOMBUF_VIEW(YDBUFS, YLVARS, INR, ZCOMBUFR, KPTRGP, YLGV)
