...
MODULE SPNSDE_VIEW_MOD
CONTAINS
SUBROUTINE SPNSDE_VIEW(KM,KF_SCALARS,PEPSNM,PF,PNSD)
...
!**** *SPNSDE_VIEW* - Compute North-South derivative in spectral space
...
!     Modifications.
!     --------------
!        Original : 00-02-01 From SPNSDE_VIEW in IFS CY22R1
...
DO JN=KM-1,ISMAX+2
  IJ = ISMAX+3-JN
  ZN(IJ) = REAL(F%RN(JN),JPRB)
  IF( JN >= 0 ) ZEPSNM(IJ) = PEPSNM(JN)
ENDDO
ZN(0) = REAL(F%RN(ISMAX+3),JPRB)
...
END SUBROUTINE SPNSDE_VIEW
END MODULE SPNSDE_VIEW_MOD
