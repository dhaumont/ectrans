...
TYPE(SPEC_VIEW), INTENT(IN) :: YLSPVOR(:), YLSPDIV(:)
TYPE(SPEC_VIEW), INTENT(IN) :: YLSSCALAR(:)
...
IF (KF_UV > 0) THEN
  ...
  IVL   = 6*KF_UV+1
  IVU   = 8*KF_UV
  CALL PRFI1B_VIEW(KM,ZIA(:,IVORL:IVORU),YLSPVOR,KF_UV,KFLDPTRUV)
  CALL PRFI1B_VIEW(KM,ZIA(:,IDIVL:IDIVU),YLSPDIV,KF_UV,KFLDPTRUV)
  ILAST = ILAST+4*KF_UV
...
IF(KF_SCALARS > 0)THEN
  ...
  CALL PRFI1B_VIEW(KM,ZIA(:,IFIRST:ILAST),YLSSCALAR,KF_SCALARS,KFLDPTRSC)
