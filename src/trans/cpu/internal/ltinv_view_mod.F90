...
USE ECTRANS_FIELD_VIEW_MOD, ONLY : SPEC_VIEW
...
TYPE(SPEC_VIEW), INTENT(IN) :: YLSPVVOR(:), YLSPVDIV(:)
TYPE(SPEC_VIEW), INTENT(IN) :: YLSPVSCALAR(:)
...
IF (KF_UV > 0) THEN
  ...
  IVL   = 6*KF_UV+1
  IVU   = 8*KF_UV
  CALL PRFI1B_VIEW(KM,ZIA(:,IVORL:IVORU),YLSPVVOR,KF_UV,KFLDPTRUV)
  CALL PRFI1B_VIEW(KM,ZIA(:,IDIVL:IDIVU),YLSPVDIV,KF_UV,KFLDPTRUV)
  ILAST = ILAST+4*KF_UV
...
IF(KF_SCALARS > 0)THEN
  ...
  CALL PRFI1B_VIEW(KM,ZIA(:,IFIRST:ILAST),YLSPVSCALAR,KF_SCALARS,KFLDPTRSC)
