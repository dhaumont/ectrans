...
IF (KF_UV > 0) THEN
  ...
  IDIVS = 2*KF_UV+1
  IDIVE = 4*KF_UV
  CALL UPDSPB_VIEW(KM,POA2(:,IVORS:IVORE),YDSPVVOR,KFLDPTRUV)
  CALL UPDSPB_VIEW(KM,POA2(:,IDIVS:IDIVE),YDSPVDIV,KFLDPTRUV)
  IF (KM == 0) THEN    
    DO JFLD=1,KF_UV
      YDSPVVOR(JFLD)%P(D%NASM0(0)) = 0.0_JPRB
      YDSPVDIV(JFLD)%P(D%NASM0(0)) = 0.0_JPRB
...
IF (KF_SCALARS > 0) THEN  
    ...
    CALL UPDSPB_VIEW(KM,POA1(:,IST:IEND),YDSPVSCALAR,KFLDPTRSC)  
ENDIF
#endif
!     ------------------------------------------------------------------
