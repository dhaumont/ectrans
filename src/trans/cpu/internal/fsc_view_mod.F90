...
SUBROUTINE FSC_VIEW(KGL,&
 & YLPUV,YLPSCALAR,&
 & YLPUV_EW,YLPSCALAR_NS,YLPSCALAR_EW)
...
TYPE(SPEC_VIEW), INTENT(IN) :: YLPUV(:),YLPSCALAR(:)
TYPE(SPEC_VIEW), INTENT(IN) :: YLPUV_EW(:),YLPSCALAR_NS(:), YLPSCALAR_EW(:)
...
KF_UV = SIZE(YLPUV)/2
KF_SCALARS = SIZE(YLPSCALAR)
KF_SCDERS = SIZE(YLPSCALAR_NS)
  ...
  IF( S%LSHIFTLL ) THEN
    ...
    DO JF=1,KF_SCALARS
      DO JM=0,IMEN
        ...
        ZAMP = SQRT(YLPSCALAR(JF)%P(IR)**2 + YLPSCALAR(JF)%P(II)**2)
        ZPHASE = ATAN2(YLPSCALAR(JF)%P(II),YLPSCALAR(JF)%P(IR)) + REAL(JM,JPRB)*ZSHIFT
        
        YLPSCALAR(JF)%P(IR) = ZAMP*COS(ZPHASE)
        YLPSCALAR(JF)%P(II) = ZAMP*SIN(ZPHASE)
      ENDDO
    ENDDO
    IF(KF_SCDERS > 0)THEN
      DO JF=1,KF_SCALARS
        DO JM=0,IMEN
          ...
          ZAMP = SQRT(YLPSCALAR_NS(JF)%P(IR)**2 + YLPSCALAR_NS(JF)%P(II)**2)
          ZPHASE = ATAN2(YLPSCALAR_NS(JF)%P(II),YLPSCALAR_NS(JF)%P(IR)) + REAL(JM,JPRB)*ZSHIFT
          YLPSCALAR_NS(JF)%P(IR) = ZAMP*COS(ZPHASE)
          YLPSCALAR_NS(JF)%P(II) = ZAMP*SIN(ZPHASE)
        ENDDO
      ENDDO
    ENDIF
    DO JF=1,2*KF_UV
      DO JM=0,IMEN
        ...
        II = IR+1
        ! calculate amplitude and phase shift and reconstruct A,B
        ZAMP = SQRT(YLPUV(JF)%P(IR)**2 + YLPUV(JF)%P(II)**2)
        ZPHASE = ATAN2(YLPUV(JF)%P(II),YLPUV(JF)%P(IR)) + REAL(JM,JPRB)*ZSHIFT
        YLPUV(JF)%P(IR) =  ZAMP*COS(ZPHASE)
        YLPUV(JF)%P(II) =  ZAMP*SIN(ZPHASE)
      ENDDO
    ENDDO
  ENDIF
ENDIF
...
IF(KF_UV > 0) THEN
  DO JLON=ISTAGTF+1,ISTAGTF+2*(IMEN+1)
    DO JF=1,2*KF_UV
      YLPUV(JF)%P(JLON) = YLPUV(JF)%P(JLON)*ZACHTE2
    ENDDO
  ENDDO
ENDIF
...
IF(KF_SCDERS > 0)THEN
  DO JLON=ISTAGTF+1,ISTAGTF+2*(IMEN+1)
    DO JF=1,KF_SCALARS
      YLPSCALAR_NS(JF)%P(JLON) = YLPSCALAR_NS(JF)%P(JLON)*ZACHTE2
    ENDDO
  ENDDO
ENDIF
...
IF(LUVDER)THEN
  DO JM=0,IMEN
    ...
    ZMUL = ZACHTE*JM
    DO JF=1,2*KF_UV
      YLPUV_EW(JF)%P(IR) = -YLPUV(JF)%P(II)*ZMUL
      YLPUV_EW(JF)%P(II) =  YLPUV(JF)%P(IR)*ZMUL
    ENDDO
  ENDDO
ENDIF
...
IF(KF_SCDERS > 0)THEN
  DO JM=0,IMEN
    ...
    ZMUL = ZACHTE*JM
    DO JF=1,KF_SCALARS
      YLPSCALAR_EW(JF)%P(IR) = -YLPSCALAR(JF)%P(II)*ZMUL
      YLPSCALAR_EW(JF)%P(II) =  YLPSCALAR(JF)%P(IR)*ZMUL
    ENDDO
  ENDDO
ENDIF
