...
MODULE LTDIR_VIEW_MOD
CONTAINS
SUBROUTINE LTDIR_VIEW(KM,KMLOC,KF_FS,KF_UV,KF_SCALARS,KLED2, YDSPVVOR, YDSPVDIV, YDSPVSCALAR)
...
!**** *LTDIR_VIEW* - Control of Direct Legendre transform step
...
!     ------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('LTDIR_VIEW_MOD',0,ZHOOK_HANDLE)
!     ------------------------------------------------------------------
...
ALLOCATE(ZSIA(KLED2,IDGLU))
ALLOCATE(ZAIA(KLED2,IDGLU))
...
CALL LEDIR(KM,KMLOC,IFC,IIFC,ISL,IDGLU,ZAIA,ZSIA,ZOA1,F%RW(1:R%NDGNH))
