...
TYPE TRGL_BUFFERS
  ...
  INTEGER(KIND=JPIM), POINTER :: ISENDTOT (:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IRECVTOT (:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: ISEND(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IRECV(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IINDEX(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: INDOFF(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IGPTRSEND(:,:,:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: ISETWL(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: ISETVL(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: ISETW(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IJPOS(:,:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IPOSPLUS(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IFLDA(:,:) => NULL()

END TYPE TRGL_BUFFERS

TYPE TRGL_VARS
  INTEGER(KIND=JPIM), POINTER :: IUVLEVS(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IUVPARS(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IGP2PARS(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IFLDOFF(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IGPTROFF(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IGP3APARS(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IGP3ALEVS(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IGP3BPARS(:) => NULL()
  INTEGER(KIND=JPIM), POINTER :: IGP3BLEVS(:) => NULL()

  LOGICAL, POINTER   :: LLUV(:)
  LOGICAL, POINTER   :: LLGP2(:)
  LOGICAL, POINTER   :: LLGP3A(:)
  LOGICAL, POINTER   :: LLGP3B(:)
END TYPE TRGL_VARS

CONTAINS

SUBROUTINE TRGL_BIND_BUFFERS_CST(SELF, &
                          & ISENDTOT,IRECVTOT,ISEND,IRECV, &
  ...
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: ISENDTOT (NPROC)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IRECVTOT (NPROC)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: ISEND    (NPROC)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IRECV    (NPROC)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IINDEX(D%NLENGTF)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: INDOFF(NPROC)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IGPTRSEND(2,NGPBLKS,NPRTRNS)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: ISETWL(NPROC), ISETVL(NPROC)

 ! IF (ASSOCIATED(SELF%ISENDTOT)) RETURN

  SELF%ISENDTOT => ISENDTOT
  SELF%IRECVTOT => IRECVTOT
  SELF%ISEND => ISEND
  SELF%IRECV => IRECV
  SELF%IINDEX => IINDEX
  SELF%INDOFF => INDOFF
  SELF%IGPTRSEND => IGPTRSEND
  SELF%ISETWL => ISETWL
  SELF%ISETVL => ISETVL

END SUBROUTINE TRGL_BIND_BUFFERS_CST


SUBROUTINE TRGL_BIND_BUFFERS_SR(SELF, KF_GP, KNRECV, IPOSPLUS,ISETW,IJPOS,IFLDA)
  USE TPM_TRANS       ,ONLY : NGPBLKS

  CLASS(TRGL_BUFFERS), INTENT(INOUT) :: SELF
  INTEGER(KIND=JPIM), INTENT(IN) :: KF_GP
  INTEGER(KIND=JPIM), INTENT(IN) :: KNRECV
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IPOSPLUS(KNRECV)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: ISETW(KNRECV)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IJPOS(NGPBLKS,KNRECV)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IFLDA(KF_GP,KNRECV)

  !  IF (ASSOCIATED(SELF%ISETW)) RETURN
  IF (SELF%LLTRGTOL) THEN
    SELF%ISETW => ISETW
    SELF%IJPOS => IJPOS
    SELF%IPOSPLUS => IPOSPLUS
    SELF%IFLDA => IFLDA
  ELSE
    SELF%ISETW => ISETW
    SELF%IJPOS => IJPOS
    ...
    SELF%IFLDA => IFLDA
  ENDIF
END SUBROUTINE TRGL_BIND_BUFFERS_SR

SUBROUTINE TRGL_BIND_VARS(SELF, KF_GP, KF_FS,  &
                        & IUVLEVS,  IUVPARS,  IGP2PARS,  IFLDOFF, &
                        & IGPTROFF,  LLUV,  LLGP2, &
                        & LLGP3A,  LLGP3B, &
  ...
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: KF_GP, KF_FS
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IUVLEVS(KF_GP),IUVPARS(KF_GP),IGP2PARS(KF_GP)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IGP3APARS(KF_GP),IGP3ALEVS(KF_GP),IGP3BPARS(KF_GP),IGP3BLEVS(KF_GP)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IFLDOFF(KF_FS)
  INTEGER(KIND=JPIM), TARGET, INTENT(IN) :: IGPTROFF(NGPBLKS)
  LOGICAL, TARGET, INTENT(IN)   :: LLUV(KF_GP),LLGP2(KF_GP),LLGP3A(KF_GP),LLGP3B(KF_GP)

  !  IF (ASSOCIATED(SELF%IUVLEVS)) RETURN

  SELF%IUVLEVS => IUVLEVS
  SELF%IUVPARS => IUVPARS
  SELF%IGP2PARS =>IGP2PARS
  SELF%IFLDOFF => IFLDOFF
  SELF%IGPTROFF => IGPTROFF
  SELF%LLUV =>LLUV
  SELF%LLGP2 => LLGP2
  SELF%LLGP3A => LLGP3A
  SELF%LLGP3B => LLGP3B
  SELF%IGP3APARS =>  IGP3APARS
  SELF%IGP3ALEVS => IGP3ALEVS
  SELF%IGP3BPARS =>IGP3BPARS
  SELF%IGP3BLEVS => IGP3BLEVS
END SUBROUTINE TRGL_BIND_VARS
