...
USE TRGL_MOD, ONLY: TRGL_BUFFERS, TRGL_BIND_BUFFERS_CST, TRGL_PROLOG
...
INTEGER(KIND=JPIM) :: ISETWL(NPROC), ISETVL(NPROC)

TYPE(TRGL_BUFFERS),  SAVE :: YDBUFS
...
CALL TRGL_BIND_BUFFERS_CST(YDBUFS, ISENDTOT,IRECVTOT,ISEND,IRECV, &
                          & IINDEX,INDOFF,IGPTRSEND, &
                          & ISETWL,ISETVL)
CALL GSTATS(1806, 0)
...
USE TPM_TRANS       ,ONLY :  NGPBLKS
USE TRGL_MOD, ONLY: TRGL_BUFFERS, TRGL_VARS, TRGL_BIND_VARS, TRGL_ALLOCATE_HEAP_BUFFER, &
  &                 TRGL_INIT_VARS, TRGL_INIT_OFF_VARS, TGRL_COPY_ZCOMBUF, TGRL_COPY_PGLAT, &
  &                 TGRL_INIT_PACKING_VARS, TRGL_BIND_BUFFERS_SR
...
INTEGER(KIND=JPIM) :: IPOSPLUS(YDBUFS%INRECV)
INTEGER(KIND=JPIM) :: ISETW(YDBUFS%INRECV)
INTEGER(KIND=JPIM) :: IJPOS(NGPBLKS,YDBUFS%INRECV)
INTEGER(KIND=JPIM) :: IFLDA(KF_GP,YDBUFS%INRECV)
...
CALL TRGL_BIND_BUFFERS_SR(YDBUFS, KF_GP, KNRECV, IPOSPLUS,ISETW,IJPOS,IFLDA)
...
CALL TRGL_BIND_VARS(YLVARS, KF_GP,KF_FS, IUVLEVS,  IUVPARS,  IGP2PARS,  IFLDOFF, &
                        & IGPTROFF,  LLUV,  LLGP2, &
                        & LLGP3A,  LLGP3B, &
